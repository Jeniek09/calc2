...
def test_calculator_subtract(_clear_history):
    ...
    for ind in df_1.index:
        assert Calculator.subtraction(df_1['value_1'][ind], df_1['value_2'][ind]).round(decimals=4) \
               == df_1['result'][ind].round(decimals=4)

def test_write_csv():
    """testing out write function to the test_data folder"""
    # pylint: disable=unsubscriptable-object
    read_filename = 'subtraction.csv'
    write_filename = 'subtraction_output.csv'
    ...
    df_1['output'] = Calculator.subtraction(df_1['value_1'], df_1['value_2']).round(decimals=4)
